### 👨‍🚀 Hi there, 


I'm a cybersecurity professional with a strong interest in SOC operations, threat detection, and hands-on security projects. I have experience working with tools like Splunk, Wazuh, Wireshark, Nmap, and Kali Linux, and I’m focused on continuing to build my skills in blue teaming, threat hunting, and incident response.

---

### Tools
<p>
  <img src="https://img.shields.io/badge/Splunk-000000?style=flat&logo=splunk&logoColor=white" />
  <img src="https://img.shields.io/badge/Wazuh-005571?style=flat&logo=wazuh&logoColor=white" />

  <img src="https://img.shields.io/badge/Nessus-00AEEF?style=flat&logo=tenable&logoColor=white" />
  <img src="https://img.shields.io/badge/Wireshark-1679A7?style=flat&logo=wireshark&logoColor=white" />
  <img src="https://img.shields.io/badge/Nmap-004170?style=flat" />
  <img src="https://img.shields.io/badge/Burp%20Suite-FF6633?style=flat&logo=burpsuite&logoColor=white" />
</p>

<p>
  <img src="https://skillicons.dev/icons?i=linux,kali,ubuntu,windows,powershell,bash,python,git,github" width="400" />
</p>

---

### Featured Projects

- [Detection Engineering Sigma/Wazuh Lab](https://github.com/josue6368/detection-engineering-sigma-wazuh-lab) — Built SIEM detections for failed logons, PowerShell discovery, and Active Directory group changes.
- [Malware Traffic Analysis Lab](https://github.com/josue6368/malware-traffic-analysis-lab) — Analyzed PCAP traffic to identify suspicious DNS, HTTP, beaconing, and command-and-control indicators.
- [Active Directory Attack & Defense Lab](https://github.com/josue6368/active-directory-attack-defense-lab) — Simulated identity-based attacks and analyzed Windows security logs.
- [OWASP Juice Shop Web App Pentest](https://github.com/josue6368/owasp-juice-shop-web-app-pentest) — Tested SQL injection, XSS, broken access control, and IDOR vulnerabilities.
- [Nessus Vulnerability Management Lab](https://github.com/josue6368/nessus-vulnerability-management-lab) — Performed vulnerability scanning, risk analysis, and remediation documentation.
- [Wazuh SIEM Home Lab](https://github.com/josue6368/Wazuh-SIEM-Home-Lab-Mini-SOC-Environment) — Built a mini SOC lab with Wazuh, Ubuntu Server, Kali Linux, and a Windows endpoint.
<br>
<p align="center">
  <a href="https://github.com/josue6368?tab=repositories">
    <img src="https://img.shields.io/badge/Full%20Project%20List-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

### Connect

<p>
  <a href="https://www.linkedin.com/in/josuecharry/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white" />
  </a>
</p>
